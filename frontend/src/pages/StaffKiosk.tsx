...
import { Clock, CheckCircle, LogOut, ArrowLeft, Loader } from 'lucide-react';
...
const StaffKiosk: React.FC = () => {
  ...
  const handleSubmit = async (type: 'In' | 'Out') => {
    ...
    if (!user?.propertyId) {
      setStatus('error');
      setErrorMessage('Kiosk not linked to a property.');
      return;
    }
    ...
    if (error || !data) {
      ...
      setStaffName(data.firstname || 'Staff Member');
      setAction(type);
      setStatus('success');

      // In a real app, we would insert a TimeClock record here
      // await supabase.from('time_clocks').insert({ staff_id: data.id, type: type, timestamp: new Date() });
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-emerald-500 text-white p-12 rounded-2xl shadow-2xl text-center max-w-md w-full animate-in zoom-in duration-300">
          ...
          <p className="text-xl opacity-90">Welcome, {staffName}</p>
          <p className="text-lg mt-2">Clocked {action} at {new Date().toLocaleTimeString()}</p>
        ...
        <div className="max-w-4xl w-full grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <Card className="h-full flex flex-col justify-center items-center p-12 bg-white border-none shadow-xl">
              ...
              {status === 'error' && <p className="mt-4 text-red-500 font-bold animate-pulse">{errorMessage}</p>}
            </Card>
          </div>
          ...
          <Card className="p-8 border-none shadow-xl bg-slate-50">
            <div className="mb-8">
              ...
              <div className="flex justify-center gap-4">
                {[...Array(4)].map((_, i) => (
                  <div key={i} className={`w-12 h-16 rounded-lg border-2 flex items-center justify-center text-3xl font-bold transition-all ${pin[i]
